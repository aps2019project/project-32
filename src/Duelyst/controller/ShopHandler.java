...
public class ShopHandler {
                            ...
                            checkBox.setOnMouseClicked(new EventHandler<MouseEvent>() {
                                ...
                                public void handle(MouseEvent event) {
                                    if (checkBox.isSelected()) {
                                        ...
                                        if (Player.getLoginAccount().getCollection().getHeroes().contains(jfxTextField.getText()))
    ...
    public void goMainMenu(KeyEvent event) {
        ...
        for (VBox vBox : heroBoxes) {
            if (((CheckBox) vBox.getChildren().get(0)).isSelected()) {
                String cardName = ((Label) (vBox.getChildren().get(2))).getText().split("\\n")[0]
                        .replaceAll("\\s", "").toLowerCase();
                if (Player.getLoginAccount().getCollection().hasThisCard(cardName)) {
                    Player.getLoginAccount().getCollection().removeCardFromCollection(cardName);
                    Player.getLoginAccount().incrementMoney(shop.costOfCard(cardName));
    ...
    public void buyBtnAct() {
        ...
        for (VBox vBox : heroBoxes) {
            if (((CheckBox) vBox.getChildren().get(0)).isSelected()) {
                ...
                if (!Player.getLoginAccount().getCollection().hasThisCard(string)) {
                    if (shop.costOfCard(string) <= Player.getLoginAccount().getMoney()) {
                        Player.getLoginAccount().getCollection().addToCollection(string);
                        Player.getLoginAccount().incrementMoney(-shop.costOfCard(string));
