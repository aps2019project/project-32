...
import client.models.TimeHandler;
...
{
    protected Battle(Player firstPlayer, Player secondPlayer, int winnerPrize) throws InvalidDeck, CloneNotSupportedException
    {
        ...
        public void removeDeadCardFromMap(Card intendedDeadCard)
        {
            for (int i = 0; i < 5; i++)
                for (int j = 0; j < 9; j++)
                    ...
                    {
                        Controller.getInstance().getCurrentPlayer().getGraveYard().getGraveYardList().add(intendedDeadCard);
...
//                ((Minion) intendedCard).getSpecialSpell().generalDo(battleMap, battleMap.getPosition(minion));
            ...
            ((Warrior) intendedCard).attackTiredAffect();
            ((Warrior) intendedCard).moveTiredAffect();
    ...
    {
        battleMap.warriorsOnMap[2][0] = firstPlayer.getCopiedMainDeck().getHero();
        battleMap.warriorsOnMap[2][8] = secondPlayer.getCopiedMainDeck().getHero();
    }
...
//        checkDeadActions();
    ...
    public void winActions(Player player)
    {
        ...
        Controller.getInstance().getStage().setScene(MainMenu.getInstance().getScene());
        Controller.getInstance().getCurrentPlayer().getPlayerHand().getHandCards().clear();
    }
    ...
    public void moveWarriorOptions(Warrior intendedWarrior, Position sourcePosition, Position destinationPosition) throws CantMove
    {
        ...
        if (widget != null && (widget.getType() == Type.Collectible || widget.getType() == Type.Flag))
    ...
    private void checkDeadActions() throws Exception
    {
        for (int i = 0; i < 5; i++)
            for (int j = 0; j < 9; j++)
            {
                ...
                {
                    String voice = "C:/Users/hp/Desktop/my own project/Css/pictures/New folder (3)/sfx/sfx_f2_stormkage_death.m4a";
